...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if ((ourAnimals[i, 2] != "" || ourAnimals[i, 2] != "?") && int.TryParse(ourAnimals[i, 2], out _))
                {
                    do
                    {
                        Console.WriteLine($"Enter an age for {ourAnimals[i, 0]}");
                        readResult = Console.ReadLine();
                        if (int.TryParse(readResult, out int result))
                        {
                            animalAge = readResult;
                            validEntry = int.TryParse(animalAge, out petAge);
                        }
        ...
        case "4":
            ...
            Console.WriteLine("\n\rNickname and physical description fields are complete for all of our friends. \n\rPress the Enter key to continue");
            readResult = Console.ReadLine();
            break;
        case "5":
